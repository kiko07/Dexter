...
class _FileGroup {
  ...
  _FileGroup({required this.fileName, required this.batches});

  int get totalRecords => batches.fold(0, (sum, b) => sum + b.rowCount);
  int get importCount => batches.length;
  DateTime get firstImport => batches.map((b) => b.importedAt).reduce((a, b) => a.isBefore(b) ? a : b);
  DateTime get lastImport => batches.map((b) => b.importedAt).reduce((a, b) => a.isAfter(b) ? a : b);
}
...
class _ManageFilesScreenState extends ConsumerState<ManageFilesScreen> {
  ...
  Future<void> _loadBatches() async {
    ...
    final Map<String, List<ImportBatch>> grouped = {};
    for (final batch in batches) {
      grouped.putIfAbsent(batch.originalFileName, () => []).add(batch);
    }

    final groups = grouped.entries
        .map((e) => _FileGroup(fileName: e.key, batches: e.value))
        .toList()
      ..sort((a, b) => b.lastImport.compareTo(a.lastImport)); // newest first
  ...
  Future<void> _deleteFileGroup(_FileGroup group) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(AppLocalizations.of(context)!.confirmDeleteImportBatch(group.fileName)),
        actions: [
          TextButton(
    ...
    // Delete all batches and their entries for this file
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context)!.deletedSuccessfully(group.fileName)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)!.manageIndexedFiles, style: const TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
      body: _isLoading
          ...
          : _groups == null || _groups!.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.folder_off_rounded, size: 64, color: theme.colorScheme.onSurface.withValues(alpha: 0.2)),
                      const SizedBox(height: 16),
                      Text(AppLocalizations.of(context)!.noIndexedFilesFound, style: TextStyle(color: theme.colorScheme.onSurface.withValues(alpha: 0.5))),
                    ],
                  ),
                )
              : ListView.builder(
                        ...
                        child: Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Icon(Icons.insert_drive_file_rounded, color: theme.colorScheme.primary, size: 28),
                            ),
